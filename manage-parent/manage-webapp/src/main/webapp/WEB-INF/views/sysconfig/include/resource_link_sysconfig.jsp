<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<link href="${resourceRoot}/css/bootstrap.min.css" rel="stylesheet">
<link href="${resourceRoot}/css/highlight.css" rel="stylesheet">
<link href="${resourceRoot}/css/bootstrap-switch.min.css" rel="stylesheet">
<link href="${resourceRoot}/css/global.css" rel="stylesheet">
<script src="${resourceRoot}/js/jquery.min.js"></script>
<script src="${resourceRoot}/js/bootstrap.min.js"></script>
<script src="${resourceRoot}/js/highlight.js"></script>
<script src="${resourceRoot}/js/bootstrap-switch.min.js"></script>
<script src="${resourceRoot}/js/main.js"></script>
<script type="text/javascript">
	var context = '${pageContext.request.contextPath}';
</script>
<script src="${resourceRoot}/js/scripts.js"></script>