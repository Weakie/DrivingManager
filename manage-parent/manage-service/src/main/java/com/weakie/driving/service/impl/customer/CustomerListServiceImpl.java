...
public class CustomerListServiceImpl implements CustomerListService {
	...
	public List<CustomerDetail> getIndividualCustomer(PageControl p) {
		...
		for (int i = 0; i < 10; i++) {
			details.add(new CustomerDetail("123" + i + (p.getPageIndex() - 1) * p.getPageNum(), "易P米" + i
					+ (p.getPageIndex() - 1) * p.getPageNum(), "补单生成", "13867541234" + i, true, 20, "易米",
	...
	public List<CustomerDetail> getEnterpriseCustomer(PageControl p) {
		List<CustomerDetail> details = new ArrayList<CustomerDetail>();
		for (int i = 0; i < 10; i++) {
			details.add(new CustomerDetail("234" + i + (p.getPageIndex() - 1) * p.getPageNum(), "易Q米" + i
					+ (p.getPageIndex() - 1) * p.getPageNum(), "补单生成", "13867541234" + i, true, 20, "易米",
	...
	public List<CustomerDetail> getVIPCustomer(PageControl p) {
		List<CustomerDetail> details = new ArrayList<CustomerDetail>();
		for (int i = 0; i < 10; i++) {
			details.add(new CustomerDetail("345" + i + (p.getPageIndex() - 1) * p.getPageNum(), "易E米" + i
					+ (p.getPageIndex() - 1) * p.getPageNum(), "补单生成", "13867541234" + i, true, 20, "易米",
					CustomerType.VIP));
		}
		p.setTotalNum(100);
	...
	public List<CustomerDetail> searchCustomer(PageControl p, String q) {
		List<CustomerDetail> details = new ArrayList<CustomerDetail>();
		for (int i = 0; i < 10; i++) {
			details.add(new CustomerDetail("121" + i + (p.getPageIndex() - 1) * p.getPageNum(), "易D米" + i
					+ (p.getPageIndex() - 1) * p.getPageNum(), "补单生成", "13867541234" + i, true, 20, "易米",
					CustomerType.INDIVIDUAL));
		}
		p.setTotalNum(100);
